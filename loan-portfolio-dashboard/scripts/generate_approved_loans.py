...
Reads loan_applications.xlsx, applies BRE eligibility rules from Loan Eligibility Rules.xlsx,
...
Eligibility rules are loaded from Loan Eligibility Rules.xlsx (build_loan_eligibility_rules.py).
Both files fall back to built-in defaults if not found.
...
Usage: python generate_approved_loans.py
       python generate_approved_loans.py --input   "path/loan_applications.xlsx"
                                          ...
                                          --rules   "path/Loan Eligibility Rules.xlsx"
...
parser.add_argument("--rules",   default=r"C:\Users\joshm\OneDrive\Documents\BRE\Loan Eligibility Rules.xlsx")
...
# ── 1c. LOAD ELIGIBILITY RULES ────────────────────────────────────────────────
# Built-in defaults (used if rules file is missing or a rule is disabled)
_DEFAULTS = {
    "R001": {"op": "IN",  "value": "Personal",                    "applies": "All"},
    "R002": {"op": ">",   "value": "750",                         "applies": "All"},
    "R003": {"op": "<",   "value": "0.20",                        "applies": "Salaried"},
    "R004": {"op": "<",   "value": "0.15",                        "applies": "Self-Employed,Business Owner"},
    "R005": {"op": "<",   "value": "1500000",                     "applies": "All"},
    "R006": {"op": "IN",  "value": "Tier 1,Tier 2,Tier 3",        "applies": "All"},
    "R007": {"op": ">=",  "value": "25",                          "applies": "All"},
    "R008": {"op": "<=",  "value": "50",                          "applies": "All"},
}

bre = dict(_DEFAULTS)  # start from defaults; override with file values if present

if os.path.exists(args.rules):
    try:
        rules_df = pd.read_excel(args.rules, sheet_name="Eligibility Rules", header=3)
        rules_df.columns = [c.strip().replace(" ✏", "") for c in rules_df.columns]
        rules_df = rules_df.dropna(subset=["Rule ID"])
        for _, row in rules_df.iterrows():
            ...
            if rid in bre:
                bre[rid] = {
                    ...
                    "applies": str(row["Applies To"]).strip(),
                }
        print(f"Eligibility rules loaded from {args.rules}")
    except Exception as e:
        print(f"WARNING: Could not read eligibility rules ({e}) — using built-in defaults")
else:
    print(f"WARNING: Eligibility rules file not found at {args.rules} — using built-in defaults")

def _parse_in_list(value_str):
    return [v.strip() for v in value_str.split(",") if v.strip()]

def _applies_to_row(applies_str, emp_type):
    """Return True if this rule applies to the given employment type."""
    if applies_str.strip().lower() == "all":
        return True
    return emp_type in _parse_in_list(applies_str)

# ── 2. APPLY BRE RULES (driven by Loan Eligibility Rules.xlsx) ────────────────
# R001 — Loan Product
r1 = bre["R001"]
df["loanTypeCheck"] = df["Loan_Product"].isin(_parse_in_list(r1["value"])).astype(int)

# R002 — CIBIL Score
r2 = bre["R002"]
df["cibilCheck"] = (df["CIBIL_Score"] > float(r2["value"])).astype(int)

# R003 — FOIR Salaried
r3 = bre["R003"]
sal_types = _parse_in_list(r3["applies"])
df["foirSalariedCheck"] = (
    df["Employment_Type"].isin(sal_types) & (df["FOIR"] < float(r3["value"]))
).astype(int)

# R004 — FOIR Non-Salaried
r4 = bre["R004"]
non_sal_types = _parse_in_list(r4["applies"])
df["foirNonSalariedCheck"] = (
    df["Employment_Type"].isin(non_sal_types) & (df["FOIR"] < float(r4["value"]))
).astype(int)

df["foirCheck"] = ((df["foirSalariedCheck"] == 1) | (df["foirNonSalariedCheck"] == 1)).astype(int)

# R005 — Loan Amount
r5 = bre["R005"]
df["loanAmountCheck"] = (df["Loan_Amount_Requested"] < float(r5["value"])).astype(int)

# R006 — City Tier
r6 = bre["R006"]
df["cityTierCheck"] = df["City_Tier"].isin(_parse_in_list(r6["value"])).astype(int)

# R007 / R008 — Age
r7, r8 = bre["R007"], bre["R008"]
df["ageCheck"] = ((df["Age"] >= int(float(r7["value"]))) &
                  (df["Age"] <= int(float(r8["value"])))).astype(int)

# Print the active rules so it's clear what was applied
print(f"\n  Active BRE rules applied:")
print(f"    R001 Loan Product  : {bre['R001']['value']}")
print(f"    R002 CIBIL Score   : > {bre['R002']['value']}")
print(f"    R003 FOIR Salaried : < {float(bre['R003']['value']):.0%}")
print(f"    R004 FOIR Non-Sal  : < {float(bre['R004']['value']):.0%}")
print(f"    R005 Max Loan Amt  : Rs {float(bre['R005']['value']):,.0f}")
print(f"    R006 City Tiers    : {bre['R006']['value']}")
print(f"    R007 Min Age       : {bre['R007']['value']}")
print(f"    R008 Max Age       : {bre['R008']['value']}")
print()
